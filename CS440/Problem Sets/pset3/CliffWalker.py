...
def play(q_values, env, policy):
    state = env.reset()
    done = False

    while not done:    
        # Select action
        action = policy(q_values, state)
        # Do the action
...
class GridWorld:
    ...
    def render(self, q_values=None, action=None, max_q=False, colorize_q=False):
        ...
        if action is not None:
            self.ax.set_title(self.actions_[action], color='r', weight='bold', fontsize=32)
